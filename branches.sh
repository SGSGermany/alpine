...
echo + "BRANCHES_GLOBAL=\"\$(jq -re '.Tags[]|select(test(\"^[0-9]+\\.[0-9]+$\"))' <<< \"\$BRANCHES_GLOBAL_JSON\" | sort_semver)\"" >&2
BRANCHES_GLOBAL="$(jq -re '.Tags[]|select(test("^[0-9]+\\.[0-9]+$"))' <<< "$BRANCHES_GLOBAL_JSON" | sort_semver)"
...
if [ -n "$BRANCHES_MISSING" ]; then
    echo "Explicit build instructions for the following Alpine branches are missing" >&2
